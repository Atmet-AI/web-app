...
export async function GET(
  ...
  const { data, error } = await supabase
    ...
    .select("automation:automation_id(id, name, status, description)")
    .eq("chat_id", chatId)
...
export async function POST(
  ...
  const { error } = await supabase.from("chats_automation").insert({
    chat_id: chatId,
    automation_id: parsed.data.id,
  })

  if (error) {
    if (error.code === "23505") return Errors.conflict("Automation is already linked to this chat.")
...
export async function DELETE(
  ...
  await supabase
    ...
    .delete()
    .eq("chat_id", chatId)
    .eq("automation_id", parsed.data.id)
